const educationPointCRS = require("../services/EducationCRSPoint.service");

const getAll = async (req, res) => {
  try {
    const all = await educationPointCRS.getAllEducationPoints();
    if (all.length === 0 ) {
...
const create = async (req, res) => { 
  try {
    ...
    const data = req.body;
    const created = await educationPointCRS.createEducationPoint(data);
...
const update = async (req, res) => {
  try {
    ...
    const updated = await educationPointCRS.updateEducationPoint(id, req.body);
...
const remove = async (req, res) => {
  try {
    const { id } = req.params;
    const deleted = await educationPointCRS.deleteEducationPoint(id);
